/**
 * NANP US area code → state mapping, for geo-targeting prospects "by area
 * code" as the operator asked. Not exhaustive of every overlay, but covers the
 * vast majority of active US geographic area codes. Non-geographic codes (800,
 * 888, 877, 866, 855, 844, 833, 900) are intentionally excluded.
 */
...
/** State (2-letter) for a US area code, or undefined if unknown/non-geographic. */
export function stateForAreaCode(code: string): string | undefined {
  const digits = code.replace(/\D/g, "").slice(0, 3);
  return AREA_CODE_STATE[digits];
}

/** Extract the area code from a phone number string ("+1 (602) 555-0148" → "602"). */
export function areaCodeFromPhone(phone: string): string | undefined {
  const digits = phone.replace(/\D/g, "");
  // Strip a leading US country code.
  const national = digits.length === 11 && digits.startsWith("1") ? digits.slice(1) : digits;
  if (national.length < 3) return undefined;
  return national.slice(0, 3);
}

/** Map a list of area codes to a de-duplicated list of state codes. */
export function statesForAreaCodes(codes: string[]): string[] {
  const out = new Set<string>();
  for (const c of codes) {
    const st = stateForAreaCode(c);
    if (st) out.add(st);
  }
